...
def run_cover_letter_generation(
    resume_text: str,
    job_description: str,
    previous_analysis_raw: str,
    enhancement_data_raw: str,
) -> dict:
    if not resume_text or not resume_text.strip():
        return {"success": False, "error": "Please upload your resume before generating a cover letter."}
    ...
    if not previous_analysis:
        return {"success": False, "error": "Please run an ATS analysis before generating a cover letter."}

    enhancement_data = None
    if enhancement_data_raw:
        try:
            enhancement_data = json.loads(enhancement_data_raw)
        except json.JSONDecodeError:
            enhancement_data = None

    return generate_cover_letter(resume_text, jd_result["text"], previous_analysis, enhancement_data)